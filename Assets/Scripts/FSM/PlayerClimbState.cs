...
public class PlayerClimbState : PlayerState
{
    ...
    {
        base.Enter();
        //Kill all momentum so we don't slide up the ladder
        player.body.velocity = Vector2.zero; 
        
        //Turn off gravity so we don't slide down
    ...
    public override void LogicUpdate()
    {
        ...
        //Apply Vertical Movement (and lock horizontal movement if desired)
        player.body.velocity = new Vector2(0, yInput * player.climbSpeed);
        //Set the climbing animation speed
        player.anim.SetFloat("climbVelocity", Mathf.Abs(yInput));

        //If we jump off the ladder
        ...
        //If we reach the top or bottom and leave the climbable zone
    ...
    {
        base.Exit();
        //Turn gravity back on when we leave this state
